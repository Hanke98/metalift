...
function domap (data: seq<int>, i0: int, i: int, loop0: bool) : seq<(int, int)>
  requires 0 <= i < |data|
  ensures domap(data, i0, i, loop0) == [(0,data[i]),(1,1)]
{
  [(0,data[i]),(1,1)]
}
...
lemma Lemma (data: seq<int>, sum: int, sum0: int, count: int, count0: int, i: int, i0: int, loop0: bool)
  
  requires loopInvariant(data,sum,0,count,0,i,0,loop0) && (i<|data|)
  
{
  ...
  assert doreduce(domap(data, i0, i, loop0),0) == data[i];
	...
	assert doreduce(domap(data, i0, i, loop0),1) == 1;
	Lemma2(domap(data, i0, i, loop0),mapper(data, i0, i, loop0),1);
	assert doreduce(mapper(data, i0, i+1, loop0),1) == doreduce(mapper(data, i0, i, loop0),1) + doreduce(domap(data, i0, i, loop0),1);

	
}

predicate loopInvariant (data: seq<int>, sum: int, sum0: int, count: int, count0: int, i: int, i0: int, loop0: bool)
  
{
  0 <= i <= |data| &&
	sum == doreduce(mapper(data,i0,i,loop0),0) &&
	count == doreduce(mapper(data,i0,i,loop0),1)
}

predicate postCondition (data: seq<int>, sum: int, sum0: int, count: int, count0: int, i: int, i0: int, loop0: bool)
  ...
  
{
  ...
  assert loopInvariant(data,0,0,0,0,0,0,loop0);
	...
	if(loopInvariant(data,sum,0,count,0,i,0,loop0) && (i<|data|))
	{
		Lemma(data,sum,0,count,0,i,0,loop0);
		var ind_count := count;
		ind_count := (count+1);
		var ind_i := i;
		ind_i := (i+1);
		var ind_sum := sum;
		ind_sum := (sum+data[i]);
		assert loopInvariant(data,ind_sum,0,ind_count,0,ind_i,0,loop0);
	}

	if(loopInvariant(data,sum,0,count,0,i,0,loop0) && !(i<|data|))
	{
		assert postCondition(data,sum,0,count,0,i,0,loop0);
	}
}